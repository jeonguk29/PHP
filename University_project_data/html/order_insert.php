<?
   ...
   $query="select no47 from jumun where jumunday47=curdate() order by no47 desc limit 1";
   ...
   for($i=1;$i<=$n_cart;$i++){
      ...
      $price1=round($row["price47"]*(100-$row["discount47"])/100, -3);
      $price2=$price1*$num;


         $cash = round($row["price47"]*(100-$row["discount47"])/100,-3);
         $query="insert into jumuns (jumun_no47, product_no47, num47, price47, cash47, discount47, opts_no1, opts_no2) values ('$jumun_no', $no, $num, $price1,$price2, $row[discount47], $opts1, $opts2);";
   ...
   if($product_nums>1){
      $tmp=$product_nums;
      $tmp=$tmp-1;
      $product_names=$product_names . " and " . $tmp;
   }

   if($total_cash<$max_baesongbi){
      $query="insert into jumuns (jumun_no47, product_no47, num47, price47, cash47, discount47, opts_no1, opts_no2) values ($jumun_no, 0, 1, $baesongbi, $baesongbi, 0, 0, 0);";
      $result=mysqli_query($db,$query);
      if (!$result) exit("에러: $query");
   ...
   $query="insert into jumun (no47, member_no47, jumunday47, product_names47, product_nums47, o_name47, o_tel47, o_phone47, o_email47, o_zip47, o_juso47, r_name47, r_tel47, r_phone47, r_email47, r_zip47, r_juso47, memo47, pay_method47, card_okno47, card_halbu47, card_kind47, bank_kind47, bank_sender47, total_cash47, state47)
                        values ('$jumun_no', $member_no, '$jumunday', '$product_names', $product_nums, '$o_name', '$o_tel', '$o_phone', '$o_email', '$o_zip', '$o_juso', '$r_name', '$r_tel', '$r_phone', '$r_email', '$r_zip', '$r_juso', '$memo', $pay_method, '123447789', $card_halbu, $card_kind, $bank_kind, '$bank_sender', $total_cash, 1);";
